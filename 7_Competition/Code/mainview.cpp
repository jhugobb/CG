...
void MainView::updateProjectionMatrix()
{
    ...
    projMatrix.rotate(-perspectiveRotation, 0, 1, 0); //rotates the eye to direction of the focus point from the perspective of the XoZ plane
    ...
    projMatrix.rotate(perspectiveHeight - 90, perp.x(), perp.y(), perp.z()); //rotates the eye to point to the focus point.
...
void MainView::loadSokoban(QString dir)
{
    sokoban = Sokoban(dir);
    QMatrix4x4 matrix;
...
void MainView::paintGL() {
    ...
    for (int i = 0 ; i < MODELINDEX::COUNT ; i++)
    {
        for (QMatrix4x4 matrix : objectMatrixes[i])
        {
